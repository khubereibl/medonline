...
/**
 * Display Posts Shortcode, Featured (Sticky) Posts
 * Only display featured posts using [display-posts is_featured="1"].
 *
 * TODO: Move this into the Display Posts Shortcode Additions plugin.
 */
function medonline_display_posts_featured( $args, $atts ) {
	if( isset( $atts['is_featured'] ) ) 
		$args['post__in'] = get_option( 'sticky_posts' );
	return $args;
}
add_filter( 'display_posts_shortcode_args', 'medonline_display_posts_featured', 10, 2 );
...
/**
 * Remove "Weiterlesen ->" of excerpts. (Frank, 20.05.2014)
 */
function medonline_excerpt_more( $more ) {
	return '';
}
// Remove the parent theme's excerpt_more definition:
function medonline_child_theme_setup() {
	remove_filter( 'excerpt_more', 'waipoua_auto_excerpt_more' );
	remove_filter( 'get_the_excerpt', 'waipoua_custom_excerpt_more' );
}
// Do it after the parent theme is loaded (this file is executed before the parent runs).
add_action( 'after_setup_theme', 'medonline_child_theme_setup' );
// Finally let the child play.
add_filter('excerpt_more', 'medonline_excerpt_more');
